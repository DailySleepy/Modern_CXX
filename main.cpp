...
namespace _array
{
	...
	void main()
	{
		...
		cout << boolalpha
			<< is_same_v<int[1], int[2]> << endl; // 数组是类型, 包含大小
	   ...
	   // 指向整个数组的指针（类型为 int (*)[5]），而不是指向首元素的指针 int*
		...
		int n = 10;
		{
			...
			cout << p->data << endl;
			free(p);
			// 因为 FArray 是 Plain Old Data(POD) 类型, 所以可以使用简单的 malloc /
			// free
		}
		{
			cout << "new:" << endl;
			void* mem =
				::operator new(sizeof(FArray) +
					n * sizeof(char)); // 分配原始内存，不调用构造函数。
			auto* p = new (mem) FArray; // 使用定位 new 在已分配的内存 mem 上构造
										// FArray 对象, 不进行内存分配
			p->size = n;
			strcpy_s(p->data, 10, "123456789");
			cout << p->data << endl;
...
namespace _lambda
{
	...
	void main()
	{
		struct X : decltype([] { cout << "666" << endl; }) {
		}; // lambda 是一个类 (函数对象)
		X x;
		x();

		// auto l1 = [num = 1]() { num++; }; // ERROR: lambda::operator() 默认是
		// const 函数
		...
		auto p4 =
			+[] {}; // '+' 帮助将 lambda 转换为函数指针, 否则视为类 (输出大小为1)
		...
		auto p5 = []()
		{
			a;
			b;
			f; // 编译期已知, 可以直接替换, 不是 ODR-use
			// d; &a; &b; // ERROR: ODR-use 参数必须被捕获以保持 lambda 的闭包性
			c;
			&c;
			g;
			&g; // lambda 可以直接访问"全局变量"和"封闭作用域中的局部静态变量"
		};
...
namespace _noexcept
{
	void f() noexcept { cout << "f is called" << endl; }

	void main() { cout << boolalpha << noexcept(f()) << endl; }
}
...
namespace member_pointer
{
	...
	void main()
	{
		...
		auto b0 = bind(lambda, fp, ref(x), 3);
		b0();
		auto b1 = bind(fp, ref(x), 4);
		b1();
	...
	void test()
	{
		...
		if (!flag)
		{
			...
			x.print();
			// 0 1 2 3
			// invoke立即调用, Arg&&通用引用, 所以x, &x,
			// ref(x)的结果一样(对于数据成员指针和成员函数指针)
		}
		else
		{
			auto b1 = bind(&X::inc, x);
			b1();
			x.print();
			auto b2 = bind(&X::inc, &x);
			b2();
			x.print();
			auto b3 = bind(&X::inc, ref(x));
			b3();
			x.print();
			// 0 0 1 2
			// bind将参数都存起来, 这要求使用decay_t来退化类型(与thread相同的要求),
			// 所以传值x不会对i产生影响
		}
	}
};
...
namespace default_parameter
{
	...
	struct X
	{
// const int a = 0; // 不能作为默认参数, 因为在离开类的作用域时 const
// 变量会失效, 必须要static constexpr int a = 0; // ERROR: 非 static
// 变量属于类实例, 这与 constexpr 语义矛盾, 必须要static static int a = 0;
// // ERROR: static 变量必须是 inline 或 const/constexpr
		inline static int a =
			0; // inline 解决了之前的版本中必须在类外对 static 变量进行定义的麻烦
		const static int aa = 1;
		void f(int b = sizeof a + aa)
		{
			cout << b << endl;
		} // 默认参数必须是属于静态存储期, 即static
	};
...
// 核心:
// 1.万能引用和模板类型推导: 保留值类别, 将左值捕获为左值引用,
// T推导为U&，将右值捕获为右值引用, T推导为值类型U 2.forward 的有条件转发:
// 传递值类别, 只将右值引用转为右值, 否则转为左值 (move 无条件转发为右值)
// 解决痛点: 既用引用/移动代替拷贝, 又用一个函数代替多个重载
// 使用 T 导致不必要的拷贝, 使用 T& 又无法接受右值
// C++11前, 唯一的方法是为左值右值设计两个函数, 对于右值仍然只能用 const& 来捕获
// C++11后(不用完美转发的情况下), 可以右值引用捕获右值, 再move触发移动,
// 但是仍然需要管理多个函数
{
	template <class T>
	...
	// 防止编译器通过参数来自动推导T, 这在完美转发中是不应该被允许的,
	// 因为此时参数已经丢失了值类别 于是强制用户在调用 forward 时通过 <>
	// 来指明模板参数 T, 以顺利传递值类别这一信息 虽然去掉 remove_reference
	// 也可以手动使用 <>, 但是这不是强制的, 容易忘记
	// 且当期望转发的类型与参数的类型发生冲突时 (如参数为右值引用, 但是T为左值引用)
	// 会报错
	{
	...
	template <class T>
	constexpr T&&
		__forward(remove_reference_t<T>&& _Arg) noexcept // forward 对于右值参数的重载
	{
		cout << "r forward" << endl;
